...
assert.ok(observer.includes('isOpenCodeLike(target.backend) || target.backend === "codex"'), 'OpenCode/Mimocode/Codex list-level models must stay provisional until native turn metadata is recovered')
...
assert.ok(adapter.includes('api.listStatuses(entry.target.config, openCode && entry.target.external ? entry.target.directory : undefined)'), 'OpenCode reconciliation must read the directory-scoped session status only for external Sessions so child Sessions are not reported idle')
assert.ok(adapter.includes('if (openCode && !entry.target.external && entry.forcedStatus !== "running" && !openCodeRecoveryWatchActive) return'), 'internal idle/pre-Send OpenCode sessions must not call listStatuses (#351 contract)')
assert.ok(adapter.includes('entry.openCodeIdleObservedAt = null\n        entry.forcedStatus = "running"'), 'a busy session status must adopt a turn that was started outside the app')
...
assert.ok(adapter.includes('entry.statusType = next\n    if (!nativeSessionIsWorking(next)) entry.forcedStatus = null'), 'non-OpenCode projections must retain native status enrichment')
...
assert.ok(liveRefresh.includes('event.type === "session.error"') && liveRefresh.includes('isOpenCodeLike(target.config.backend)') && liveRefresh.includes('settleAfterLifecycle()'), 'OpenCode/Mimocode session.error must trigger the bounded lifecycle settle path')
