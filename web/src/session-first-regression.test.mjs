...
assert.ok(observer.includes('target.backend === "opencode" || target.backend === "mimocode" || target.backend === "codex"'), 'OpenCode/Mimocode/Codex list-level models must stay provisional until native turn metadata is recovered')
...
assert.ok(adapter.includes('api.listStatuses(entry.target.config, openCode ? entry.target.directory : undefined)'), 'OpenCode reconciliation must read the directory-scoped session status so child Sessions are not reported idle')
...
assert.ok(liveRefresh.includes('event.type === "session.error"') && liveRefresh.includes('target.config.backend === "opencode"') && liveRefresh.includes('settleAfterLifecycle()'), 'OpenCode/Mimocode session.error must trigger the bounded lifecycle settle path')
